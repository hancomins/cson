...
public class MutableBigIntegerMock {
    ...
    MockBigInteger toBigInteger(int sign) {
        if (intLen == 0 || sign == 0)
            return com.snoworca.cson.util.MockBigInteger.ZERO;
    ...
    void rightShift(int n) {
        ...
        if (nBits == 0)
            return;
        int bitsInHighWord = com.snoworca.cson.util.MockBigInteger.bitLengthForInt(value[offset]);
    ...
    MutableBigIntegerMock divide(MutableBigIntegerMock b, MutableBigIntegerMock quotient, boolean needRemainder) {
        if (b.intLen < com.snoworca.cson.util.MockBigInteger.BURNIKEL_ZIEGLER_THRESHOLD ||
                intLen - b.intLen < com.snoworca.cson.util.MockBigInteger.BURNIKEL_ZIEGLER_OFFSET) {
    ...
    MutableBigIntegerMock divideAndRemainderBurnikelZiegler(MutableBigIntegerMock b, MutableBigIntegerMock quotient) {
        ...
        if (r < s) {
            ...
            int m = 1 << (32-Integer.numberOfLeadingZeros(s/ com.snoworca.cson.util.MockBigInteger.BURNIKEL_ZIEGLER_THRESHOLD));
    ...
    private MutableBigIntegerMock divide2n1n(MutableBigIntegerMock b, MutableBigIntegerMock quotient) {
        ...
        if (n%2 != 0 || n < com.snoworca.cson.util.MockBigInteger.BURNIKEL_ZIEGLER_THRESHOLD) {
    ...
    MutableBigIntegerMock sqrt() {
        ...
        if (bitLength() <= 63) {
            ...
            MockBigInteger bi = com.snoworca.cson.util.MockBigInteger.valueOf((long)Math.ceil(Math.sqrt(d)));
